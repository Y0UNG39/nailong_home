...
exports.main = async (event) => {
  ...
  try {
    ...
    if (res.data.length === 0) {
      ...
      const addRes = await db.collection('users').add({ data: { _openid: OPENID, nickname: safeNick, avatar: safeAvatar, coins: 0, coupleId: null, createdAt: db.serverDate() } })
      return { success: true, openid: OPENID, isNew: true, user: { _id: addRes._id, nickname: safeNick, avatar: safeAvatar, coins: 0, coupleId: null } }
    }
    // 老用户：如果传了头像或昵称就更新
